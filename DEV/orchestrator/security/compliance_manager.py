...
Component: Compliance & Audit Manager
...
class ComplianceManager:
    ...
    def __init__(self):
        # Load secret key from environment for persistence across restarts
        # Falls back to a random key if not found (warning: breaks verification on restart)
        self.secret_key = os.getenv("UBP_SECURITY_KEY", "").encode()

        if not self.secret_key:
            logger.warning(
                "UBP_SECURITY_KEY not set. Using temporary random key for ComplianceManager."
            )
    ...
    def validate_compliance(
        self, request_data: Dict[str, Any], compliance_rules: Dict[str, Any]
    ) -> List[str]:
        ...
        if "classification" in compliance_rules:
            required_classification = compliance_rules["classification"].get(
                "level", "public"
            )

            if not self._check_classification_compliance(
                request_data, required_classification
            ):
                violations.append("invalid_data_classification")

        # Check geographic restrictions
        if "geo_restrictions" in compliance_rules:
            allowed_regions = compliance_rules["geo_restrictions"].get(
                "allowed_regions", []
            )
    ...
    def _check_retention_compliance(
        self, data: Dict[str, Any], retention_period: int
    ) -> bool:
        ...
        try:
            ...
            # Ensure creation_date is aware to compare with now(utc)
    ...
    def _check_classification_compliance(
        self, data: Dict[str, Any], required_level: str
    ) -> bool:
        """
    ...
    def _check_geo_compliance(
        self, data: Dict[str, Any], allowed_regions: List[str]
    ) -> bool:
